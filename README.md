...
The end goal is to have a custom slicer convert 3D models into LED frames that the STM32WB55 can display at the correct motor positions. Bluetooth will be used for control and content transfer, while USB-C is used only for battery charging and external power. The platform can display general volumetric graphics or operate as an electronic hourglass by linking its animations to a timer.
